...
var resolve$ref = Promise.method(function(val, base$id){
  ...
  if (smap[refArr.startpart]){
    ...
    if (pointer.has(obj, refArr.endpart)){
      ...
      var x = processISchema(ischema, refArr.startpart);
      return x;
    }
  }
});
...
var processFurther = Promise.method(function(val, key, $id){
  ...
  if (val['$ref']){
    ...
    val['links'].forEach(l => {
      console.log('processFurther:', l)
      resolve$ref(l).then(s => {
        console.log('test:', s);
      })
    })
  } else {
...
var processLink = Promise.method(function(val, key, $id){
  
  val.$name =  val.title || val.rel;
  if (val.submissionSchema) {
    resolve$ref(val.submissionSchema).then((s,b) => {
      console.log('submission:', s,b);
      console.log('submission:', arguments);
    })
  }
  
  if (val.targetSchema) {
    resolve$ref(val.targetSchema).then(s => {
      console.log('targetSchema:', s);
    })
  }
  return val;
});
...
function processSchema(schema){
  return new Promise((resolve, reject) => {
    ...
    if (schema['allOf']){
      ...
      if (schema['links']) {
        _.forOwn(schema['links'], (val, key) => {
          processLink(val, base$id).then(pfSchema => {
            if (pfSchema){
              val = pfSchema;
            }
          });
